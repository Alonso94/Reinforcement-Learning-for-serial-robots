...
class TRPO(object):
    def __init__(self,env):
        ...
        self.optimize_policy=self.optimizer.minimize(self.neg_policy_loss,var_list=self._policy_model_params.append(self.sigma))
    ...
    def policy_loss_function(self):
        ...
        negloss = -tf.reduce_mean(self.advantages_placeholder * tf.exp(self.logp - self.prev_logp))
    ...
    def sample_action(self, session, input):
        return session.run(self.sampled_action, feed_dict = {self.input_placeholder:np.reshape(input, (-1, self.env_state_shape[0]))})
    ...
    def update_policy(self,session,t):
        ...
        prev_mu,prev_sigma=session.rus([self.mu,self.sigma],feed_dict)
    ...
    def update_value(self,t):
        ...
        self.value_model.fit(x=states,y=target_values,epochs=self.n_value_epochs,batch_size=self.value_batch_size,verbose=0)
    ...
    def rollout(self,session,horizon):
        ...
        self.auditor.update({'episode_number':self.episode_count,
                             'per_episode_mean':int(np.sum(np.concatenate(raw_t['rewards']))/(horizon*self.reward_scale))})
    ...
    def train(self,session):
        with session.as_default(),session.graph.as_default():
            ...
            while self.episode_count<self.horizon :
                raw_t=self.rollout(session,self.horizon)
                t_processed=self.process_trajectories(session,raw_t)
